...
export declare class FiatTransactions {
    id: string;
    type: 'BUY_ORDER_FEE' | 'CARD_PURCHASE' | 'DEPOSIT_FEE' | 'DEPOSIT_FUNDS_RECEIVED' | 'IEO_PLEDGE' | 'LOAD_PLUTUS_CARD_FROM_CJ_WALLET' | 'ORDER_CANCELLED' | 'ORDER_CREATED' | 'ORDER_EXPIRED' | 'ORDER_FULFILLED' | 'PLUTON_WITHDRAW' | 'PLUTUS_WALLET_WITHDRAW_FEE' | 'SELL_ORDER_FEE' | 'SELL_ORDER_LOAD' | 'SUBSCRIPTION_CHARGE' | 'TRANSACTION_CANCELLED' | 'UNLOAD_PLUTUS_CARD_TO_CJ_WALLET' | 'WITHDRAWAL_FEE' | 'WITHDRAWAL_REQUEST_CREATED' | 'WITHDRAWAL_REQUEST_REJECTED' | null;
    fiatAmount: string;
